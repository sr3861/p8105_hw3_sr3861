...
**Description:**
This dataset has `r nrow(ny_noaa)` observations for `r ncol(ny_noaa)` variables. The data includes variables to describe the min and max temperatures, precipitation, and snow on different dates. In the whole dataset, there are `r sum(is.na(ny_noaa))` missing values. 
